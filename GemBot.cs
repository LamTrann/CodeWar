...
namespace bot
{
    public class GemBot : BaseBot
    {
        ...
        protected override void StartTurn(ISFSObject paramz)
        {
            ...
            Hero heroFullMana = botPlayer.anyBothHeroFullMana();
            Hero enemyFullMana = enemyPlayer.anyHeroFullMana();
            Hero seaGodFullMana = botPlayer.IsSeaGodFullMana();
            Hero seaSpiritFullMana = botPlayer.IsSeaSpiritFullMana();

            var isthreeHeroAliveOrNotNearlyDead = enemyPlayer.threeHeroAliveOrNotNearlyDead();
            if (seaGodFullMana != null)
            {
                if (seaSpiritFullMana != null)
                {
                    TaskSchedule(delaySwapGem, _ => SendCastSkill(seaSpiritFullMana, isthreeHeroAliveOrNotNearlyDead));
                    return;
                }
                TaskSchedule(delaySwapGem, _ => SendCastSkill(seaGodFullMana, isthreeHeroAliveOrNotNearlyDead));
                return;
            }
            else
            {
                ...
                {
                    TaskSchedule(delaySwapGem, _ => SendCastSkill(heroFullMana, isthreeHeroAliveOrNotNearlyDead));
                    return;
                }
                else
                {
        ...
        protected override void ShowError(ISFSObject param)
        {
