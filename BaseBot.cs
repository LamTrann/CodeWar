...
namespace bot
{
    public abstract class BaseBot
    {
        ...
        private const string username = "Thien Style";

        private const string token = "eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJsYW0udHJhbnF1YW5nIiwiYXV0aCI6IlJPTEVfVVNFUiIsIkxBU1RfTE9HSU5fVElNRSI6MTY1MzQxNjI1MTMwMiwiZXhwIjoxNjU1MjE2MjUxfQ.aW7Qywa5_r8z8T67A2MWNHgtdj1M3_haavYQHjof04nL6251NAxVftxQGxBnSHgDUCIf044_Gkck63-HorE9cg";
        ...
        protected abstract void ShowError(ISFSObject param);
        ...
        public void SendCastSkill(Hero heroCastSkill, bool isthreeHeroAliveOrNotNearlyDead)
        {
            var data = new SFSObject();
            data.PutUtfString("casterId", heroCastSkill.id.ToString());
            if (isthreeHeroAliveOrNotNearlyDead)
            {
                if (heroCastSkill.isHeroSelfSkill())
                {
                    if (botPlayer.seaGodHeroAlive() && !botPlayer.seaGodHeroNearlyDead())
                    {
                        data.PutUtfString("targetId", botPlayer.affectSeagod().id.ToString());
                    }
                    else if (botPlayer.fireSpiritHeroAlive())
                    {
                        data.PutUtfString("targetId", botPlayer.affectFireSpirit().id.ToString());
                    }
                    else
                    {
                ...
                else if (botPlayer.fireSpiritHeroAlive() && botPlayer.IsFireSpiritFullMana())
                {
                    data.PutUtfString("targetId", enemyPlayer.maxAttack(grid.gems).id.ToString());
                }
                else
                {
            ...
            else
            {
                if (heroCastSkill.isHeroSelfSkill())
                {
                    if (botPlayer.seaSpiritHeroAlive())
                    {
                        data.PutUtfString("targetId", botPlayer.affectSeaSpirit().id.ToString());
                    }
                    else if (botPlayer.seaGodHeroAlive() && !botPlayer.seaGodHeroNearlyDead())
                    {
                        data.PutUtfString("targetId", botPlayer.affectSeagod().id.ToString());
                    }
                    else if (botPlayer.fireSpiritHeroAlive())
                ...
                {
                    data.PutUtfString("targetId", enemyPlayer.maxAttackForLastMoment().id.ToString());
                }
                else
                {
                    data.PutUtfString("targetId", enemyPlayer.firstHeroAlive().id.ToString());
        ...
        {
            Pair<int> indexSwap = grid.recommendSwapGem(botPlayer, enemyPlayer);
